...
/*   Updated: 2024/11/18 14:05:01 by hakaraou         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	while (y < max.y)
	{
		...
		while (x < max.x)
		{
			if (x > TILE_SIZE && x < limit && y > TILE_SIZE && y < limit)
				ft_put_pixel(cub->mlx.img, x, y, color);
			x++;
		}
		y++;
	}
...
void	player_square_draw(t_cub *cub)
{
	...
	while (y <= pos + 2)
	{
		...
		while (x <= pos + 2)
		{
			ft_put_pixel(cub->mlx.img, x, y, create_rgb(255, 0, 0));
			x++;
		}
		y++;
	}
...
void	dda(t_vec dirct, t_cub *cub, int color)
{
	...
	while (i < step * 10)
	{
		pos.x = pos.x + x_inc;
		pos.y = pos.y + y_inc;
		ft_put_pixel(cub->mlx.img, pos.x, pos.y, color);
		i++;
	}
}
